...
exports.submitExamAttempt = exports.generateAIQuestions = void 0;
 ...
 */
exports.generateAIQuestions = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "The function must be called while authenticated.");
    }
    const { subject, topic, difficulty, count } = request.data;
    if (!subject || !topic || !difficulty) {
        throw new https_1.HttpsError("invalid-argument", "Missing required parameters: subject, topic, or difficulty.");
    }
    if (difficulty !== "easy" && difficulty !== "medium" && difficulty !== "hard") {
        throw new https_1.HttpsError("invalid-argument", "Difficulty must be 'easy', 'medium', or 'hard'.");
    }
    ...
    try {
        ...
        if (!userData || (userData.role !== "faculty" && userData.role !== "admin")) {
            throw new https_1.HttpsError("permission-denied", "Unauthorized. Only faculty or admin can generate AI questions.");
        }
        const questions = await (0, gemini_1.generateMCQs)(subject, topic, difficulty, count || 10);
        ...
        throw new https_1.HttpsError("internal", error.message || "An error occurred while generating questions.");
    }
});
/**
 ...
 */
exports.submitExamAttempt = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be authenticated to submit an exam.");
    }
    const { attemptId } = request.data;
    if (!attemptId) {
        throw new https_1.HttpsError("invalid-argument", "Missing required parameter: attemptId.");
    }
    ...
    try {
        ...
        if (attemptData.userId !== request.auth.uid) {
            throw new https_1.HttpsError("permission-denied", "You can only submit your own attempt.");
        }
        if (attemptData.isSubmitted) {
            throw new https_1.HttpsError("failed-precondition", "This exam attempt has already been submitted.");
        }
        ...
        if (!examDoc.exists) {
            throw new https_1.HttpsError("not-found", "Associated exam not found.");
        }
        ...
        throw new https_1.HttpsError("internal", error.message || "An error occurred during submission.");
    }
});
